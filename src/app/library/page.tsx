...
export default async function LibraryPage({
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-[0.98rem]">
            A reduced, editorial-style library for browsing chords, scales, and neck positions.
          </p>
        </div>
  ...
  const cards = entries.map((entry) => (
    <Link
      ...
      className="card group overflow-hidden rounded-lg bg-white/[0.02] p-4 transition hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.04] sm:p-5"
    >
